<template>
	<view>
		<view class="block">
			...
			<view class="content">
				<view class="my">我的账户余额：{{ lastMoney }} 元</view>
			</view>
		</view>
		<view class="block">
			...
			<view class="content">
				<view class="amount">
					<view class="list">
						<view class="box" v-for="(amount, index) in amountList" :key="index" @tap="select(amount)" :class="{ on: amount == inputAmount }">{{ amount }}元</view>
					</view>
					<view class="num">
						<view class="text">自定义充值金额</view>
						<view class="input"><input type="number" v-model="inputAmount" /></view>
					</view>
				</view>
			</view>
		</view>
...
export default {
	...
	data() {
		...
		onLoad(option) {
			console.log(option)
			this.lastMoney = option.lastMoney;
		},
		onShow() {
			let that = this;
			uni.getStorage({
		...
		methods:{
			...
			doDeposit(){
				...
				if(!this.paymentPassword){
					...
					uni.navigateTo({
						url:'../setting/userInformation/paymentPassword'
					})
					return ;
				}
				...
				console.log(that);
				uni.request({
					url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/invest/money', //充值
					method: 'PUT',
					data: {
						userId: that.userId,
						money:that.inputAmount
					},
					success: res => {
						console.log(res.data.data);
						that.lastMoney = res.data.data;
						uni.setStorage({
							key:'lastMoney',
							data:that.lastMoney,
						})
