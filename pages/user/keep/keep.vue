<template>
	<view>
		<view class="tabr" :style="{top:headerTop}">
			<view :class="{on:typeClass=='goods'}" @tap="switchType('goods')">景点({{goodsList.length}})</view>
				...
				<view class="row" v-for="(row,index) in goodsList" :key="index" >
					...
					<view class="carrier" :class="[typeClass=='goods'?theIndex==index?'open':oldIndex==index?'close':'':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
						<view class="goods-info" @tap="toGoods(row)">
							...
							<view class="info">
								<view class="title">{{row.scenicSpotName}}</view>
								<view class="title">{{row.scenicSpotDescribe}}</view>
								<view class="price-number">
									<view class="keep-num">
										{{row.favoritePeopleNumber}}人收藏
									</view>
									<view class="price">￥{{row.adultTicketPrice}}</view>
...
export default {
	computed: {
		...
		//下拉刷新，需要page.json文件中配置开启页面下拉刷新 "enablePullDownRefresh": true
		onPullDownRefresh() {
		    setTimeout(function () {
		...
		onLoad() {
			...
			uni.request({
				url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/select/favorite/by/userId',
				method: 'POST',
				data: {
					id: that.userId
				},
				success: res => {
					...
					that.goodsList = res.data.data.favoriteUserResponseList;
			...
			deleteCoupon(id,List){
				...
				for(let i=0;i<len;i++){
					if(id==List[i].id){
						...
						uni.request({
							url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/delete/favorite',
							method: 'DELETE',
							data: {
								ids:[List[i].id]
							},
