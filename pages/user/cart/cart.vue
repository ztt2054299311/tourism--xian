	...
	<view>
		...
		<view class="goods-list">
            ...
            <view class="row" v-for="(row,index) in goodsList" :key="index" >
				<!-- 删除按钮 -->
				<view class="menu" @tap.stop="deleteGoods(row.id,goodsList)">
				...
				<view class="carrier" :class="[theIndex==index?'open':oldIndex==index?'close':'']" @touchstart="touchStart(index,$event)" @touchmove="touchMove(index,$event)" @touchend="touchEnd(index,$event)">
					...
					<view class="goods-info" @tap="toGoods(row)">
						...
						<view class="info">
							<view class="title">{{row.ticketName}}</view>
	...
	computed: {
		...
		data() {
			return {
				...
				isAllselected:false,
				goodsList:[
					// {id:1,img:'/static/img/goods/p1.jpg',name:'商品标题商品标题商品标题商品标题商品标题商品标题商品标题商品标题商品标题商品标题',spec:'规格:学生票',price:127.5,number:1,selected:false},
				],
				//控制滑动效果
				theIndex:null,
				oldIndex:null,
		...
		methods: {
			...
			getCart: function(){
				...
				return new Promise((resolve, reject) => {
					setTimeout(()=>{
						uni.request({
							url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/select/shopping/car',//查看购物车列表
							method: 'POST',
							data: {
								"userId": that.userId
							},
							success: res => {
								that.goodsList =res.data.data.orderResponseList
							},
							fail: () => {},
							complete: () => {}
						});
			...
			toGoods(e){
				...
				uni.request({
					...
					success: res => {
							uni.navigateTo({
							url: '../../details/goods?id='+ e.scenicSpotId
				});
					},
					fail: () => {},
					complete: () => {}
			...
			toConfirmation(){
				...
				console.log(ids)
				uni.request({
					url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/change/order/by/ids',
					method: 'POST',
					data: {
						ids:ids,
					},
				...
				uni.setStorage({
					...
					success: () => {
						uni.navigateTo({
							url:'../../pay/payment/payment?amount=' + this.sumPrice+'&ids='+ids
						})
					}
				})
			},
			//删除商品
			deleteGoods(id,goodsList){
				let len = this.goodsList.length;
				for(let i=0;i<len;i++){
					if(id==this.goodsList[i].id){
						...
						let that = this;
						uni.request({
							url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/delete/order',
							method: 'DELETE',
							data: {
								ids:[goodsList[i].id]
							},
			...
			// 	{
			// 		this.deleteGoods(id,this.selectedList[0]);
			// 	}
			// 	this.selectedList = [];
			...
			sub(index){
				...
				this.sum();
				uni.request({
					url: 'https://dailytravel.cn/travel-0.0.1-SNAPSHOT/update/shopCar/number',
					method: 'POST',
					data: {
