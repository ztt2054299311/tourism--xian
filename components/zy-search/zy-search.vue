	...
	<view>
		<view class="search">
			...
			<template v-if="isFocus">
				<input maxlength="20" focus type="text" value="" confirm-type="search" @confirm="searchStart()" placeholder="请输入关键词搜索" v-model.trim="searchText" :outpt = "outpt"/>
			</template>
			<template v-else>
				<input maxlength="20" type="text" value="" confirm-type="search" @confirm="searchStart()" placeholder="请输入关键词搜索" v-model.trim="searchText" :outpt = "outpt"/>
			</template>
			
			
			<image src="../../static/zy-search/search.svg" mode="aspectFit" @click="searchStart()" class="search-icon" @tap="openinfo"></image>
...
<script>
	export default{
		...
		methods: {
			...
			openinfo(e) {
				...
				var scenicSpotName = this.searchText;
				uni.navigateTo({
					url: '../details/searchList?scenicSpotName='+scenicSpotName
				});
			},
			...
			searchStart: function() {	//触发搜索
				...
				if (_this.searchText == '') {
					...
					_this.$emit(this.searchText, _this.searchText);
					uni.getStorage({
						key:'search_cache',
						success(res){
							let list = res.data;
	...
	// 		method: 'POST',
	// 		data: {scenicSpotName:'this.searchText'},
	// 		success: res => {
	// 			console.log(res);
